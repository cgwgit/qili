 ...
 //     } 
  ...
  public function detail(){
    ...
    if(!empty($post['shenqingren'])){
      ...
      $where .= " AND `shenqingren` like '{$post['shenqingren']}%'";
    ...
    $did = $_GET['id'];
      ...
      $rst = M('hetong')->where(array('bianhao' => $post['bianhao']))->find();
      if($rst){
         $err = '合同编号已存在，请重新填写';
      }
      ...
      $post['money'] > 0 ? $money = trim($post['money']) : $err = '合同金额格式不正确';
      $post['realmoney'] > 0 ? $realmoney = trim($post['realmoney']) : $err = '合同金额格式不正确';
    ...
    if(!empty($post['banben'])){
      ...
      if(session('utype') == 2){
       $where .= " AND `qiye`.`uid` =".session('uid');
        ...
        //根据页码获得数据信息出来
        $list = M('qiye')->alias('qiye')->join('tp_hetong on tp_hetong.qid=qiye.id')->where($where)->order('qiye.id desc')->select();
        //设置Excel
